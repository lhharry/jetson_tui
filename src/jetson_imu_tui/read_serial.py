...
and will likely grow again, and a block list expresses that as one more table row instead of a
new decoder branch.
...
``fb2``          2  motor_pos_r motor_pos_l -- motor position feedback
``fb6``          6  pos_r speed_r torque_r pos_l speed_l torque_l
``t``            1  the source's own clock in seconds; **must be the last block**
==========  ======  ==================================================================

``t`` is pinned last because alignment reads it as ``v[-1]``; ``_check_layouts`` enforces that at
import rather than letting a misplaced block silently shift every channel.
...
exactly as a timestamp jump does. Neither mechanism looks at what the blocks mean, so adding a
block costs nothing here beyond a longer frame.
...
sensor source the web server, recorder, CLS and the controller can consume.
...
open, so a caller that also has to *write* to the device — sending motor commands back to the
...
# Motor-feedback blocks. At most one may appear in a layout (fb6 is fb2 plus speed and torque),
# and either lands under the same "feedback" key, so consumers index it by position.
_FEEDBACK_BLOCKS = ("fb2", "fb6")
...
class Layout(NamedTuple):
    ...
    Fields:
        ...
        has_ts:   bool, True when the last block is ``t``.
...
LAYOUTS: dict[str, Layout] = {
    ...
    # --- IMU + both knees + motor feedback: what the controller needs -------------------
    # ``enable`` leads, matching Extension_jetson_Walking_ML's uplink; the feedback block
    # trails so ``t`` (where present) stays last.
    "accel_gyro_knee4_fb2": _layout("enable", "accel", "gyro", "knee4", "fb2"),
    "gyro_accel_knee4_fb2": _layout("enable", "gyro", "accel", "knee4", "fb2"),
    "accel_gyro_knee4_fb2_t": _layout("enable", "accel", "gyro", "knee4", "fb2", "t"),
    "gyro_accel_knee4_fb2_t": _layout("enable", "gyro", "accel", "knee4", "fb2", "t"),
    "accel_gyro_knee4_fb6": _layout("enable", "accel", "gyro", "knee4", "fb6"),
    "gyro_accel_knee4_fb6": _layout("enable", "gyro", "accel", "knee4", "fb6"),
    "accel_gyro_knee4_fb6_t": _layout("enable", "accel", "gyro", "knee4", "fb6", "t"),
    "gyro_accel_knee4_fb6_t": _layout("enable", "gyro", "accel", "knee4", "fb6", "t"),
}
...
def _check_layouts() -> None:
    ...
    completely silent: the classifier merely degrades and the controller merely commands
    nonsense. Refusing to import is the point — this module's standing rule is that frame
    meaning is declared rather than guessed, so a declaration that cannot be true must not load.
    """
    for name, lay in LAYOUTS.items():
        ...
        if sum(1 for b in lay.blocks if b in _FEEDBACK_BLOCKS) > 1:
            raise ValueError(f"layout {name!r}: at most one of {_FEEDBACK_BLOCKS} may appear")
...
def decode_frames(
    ...
    *,
    ...
    Yields:
        ...
        across layouts:
          "t":        float | None            -- source clock in seconds
          ...
          "feedback": list[float] len 2 or 6 | None -- motor pos (fb2), or pos/speed/torque
                      per side (fb6), device units
    ...
    fb_block = next((b for b in _FEEDBACK_BLOCKS if b in lay.offsets), None)
    off_fb = lay.offsets[fb_block] if fb_block is not None else None
    fb_len = BLOCK_SIZES[fb_block] if fb_block is not None else 0
    ...
    while stop is None or not stop.is_set():
        ...
        while len(buf) >= frame_len:
            ...
            yield {
                ...
                "feedback": list(v[off_fb:off_fb + fb_len]) if off_fb is not None else None,
            }
